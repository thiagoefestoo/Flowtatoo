...
async function ensureAdmin() {
  ...
  const [user] = await User.findOrCreate({
    where: { email: 'admin@flowtatoo.com' },
    defaults: {
      name: 'Administrador Flowtatoo',
      email: 'admin@flowtatoo.com',
      passwordHash,
      role: 'admin',
      status: 'ativo',
    },
  ...
  await user.update({ passwordHash, role: 'admin', status: 'ativo', name: user.name || 'Administrador Flowtatoo' });
}

async function main() {
  const total = Number(process.argv[2] || process.env.FLOWTATOO_VOLUME || 120);
  ...
  const paymentMethods = ['pix','dinheiro','cartao_credito','cartao_debito','transferencia'];
  ...
  for (let i = 0; i < total; i += 1) {
    ...
    let paidAmount = 0;
    if (status === 'finalizado' || Math.random() > 0.72) {
      paidAmount = price;
    } else if (deposit > 0 && Math.random() > 0.45) {
      paidAmount = Math.min(price, money(deposit, price * 0.75));
    } else if (deposit > 0) {
      paidAmount = deposit;
    }
    const paymentStatus = paidAmount >= price ? 'pago' : paidAmount > deposit ? 'parcial' : paidAmount > 0 ? 'sinal_pago' : 'pendente';
    ...
    await TattooAppointment.create({
      ...
      deposit,
      paidAmount,
      paymentMethod: paidAmount > 0 ? pick(paymentMethods) : 'nao_informado',
      paymentStatus,
      paymentNotes: paidAmount > 0 ? 'Pagamento gerado para teste do controle financeiro.' : null,
      status,
  ...
  console.log(`Flowtatoo: ${clients.length} clientes, ${artists.length} tatuadores e ${total} agendamentos gerados.`);
  console.log('Login: admin@flowtatoo.com / Admin@12345');
  await sequelize.close();
}

main().catch(async (error) => {
  console.error('Erro ao gerar volume Flowtatoo:', error);
  await sequelize.close();
  process.exit(1);
});
