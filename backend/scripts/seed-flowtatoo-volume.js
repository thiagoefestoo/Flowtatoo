...
async function main() {
  const total = Math.max(Number(process.argv[2] || process.env.FLOWTATOO_VOLUME || 50), 1);
  ...
  for (let i = 0; i < total; i += 1) {
    ...
    const email = `${name.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/\s+/g, '.')}.${stamp}.${i + 1}@flowtatoo.test`;
  ...
  console.log(`Carga Flowtatoo concluída: ${total} candidatos e registros relacionados criados.`);
  console.log('Login: admin@flowtatoo.com | Senha: Admin@12345');
  await sequelize.close();
}

main().catch(async (error) => {
  console.error('Erro ao popular volume Flowtatoo:', error);
